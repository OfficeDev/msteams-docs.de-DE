...
* Verstehen sie die für Bots relevanten Teams-App-Konfigurationen.
...
1. Wählen **Sie Create a new Bot** Create Bot  >  **Registration** aus, wie in der folgenden Abbildung dargestellt:
    ...
    Wenn der neue Bot erfolgreich ist, hat er den **Status "Registriert".** Jetzt wird Ihr Bot automatisch beim Microsoft Azure Bot Service registriert. 
...
Wenn Sie in einem anderen Lernprogramm eine Registerkarte erstellt haben, ist das App-Gerüst für den Bot anders. Im Gegensatz zu Registerkarten erfordert die Botentwicklung nicht, dass Sie Front-End-Webkomponenten erstellen oder das Teams JavaScript-Client-SDK verwenden.  Stattdessen verwendet das Gerüst das [Microsoft Bot Framework](https://dev.botframework.com/), ein Open-Source-SDK zum Erstellen intelligenter Bots auf Enterprise-Qualität, die im Web, mobil und natürlich Teams funktionieren können! 

Die Datei, die sich im Stammverzeichnis Ihres Projekts befindet, ist der teamsspezifische Handler, der Botaktivitäten verarbeitet, z. B. wie der `botActivityHandler.js` Bot auf bestimmte Nachrichten reagiert. Das App-Gerüst bietet eine Datei, die sich im Stammverzeichnis Ihres Projekts befindet, ist der spezifische Handler für Teams, der Botaktivitäten verarbeitet, z. B. wie der Bot auf `botActivityHandler.js` bestimmte Nachrichten reagiert.
...
1.  Nachdem Sie die Einrichtung abgeschlossen haben, öffnen Sie ein Terminal, und führen Sie `ngrok http -host-header=rewrite 3978` aus. 
    ...
    Ngrok stellt Ihnen nun eine öffentliche, sichere URL zur Seite, die an Ihren localhost an Port 3978 weitergeleitet wird. Kopieren Sie daher beispielsweise die HTTPS-URL, wie im screenshot unten gezeigt, da Teams HTTPS-Verbindungen `https://287a4f4223bc.ngrok.io` erfordert: 
...
Um einen Bot in Teams zu verwenden, müssen Sie ihn beim Azure Bot Service registrieren. Dies erfolgt automatisch, wenn Sie Ihre App mithilfe des Teams Toolkits einrichten.

Sie müssen weiterhin eine Endpunktadresse angeben, um Benutzernachrichten oder Anforderungen zu empfangen und zu verarbeiten, die an den Bot gesendet werden. In der Regel sieht die URL wie `https://HOST_URL/api/messages` aus. Sie können dies schnell im Toolkit konfigurieren.

1. Öffnen Visual Studio Microsoft Teams **Toolkit** in Visual Studio Code.
...
1. Geben Sie im Feld **Bot-Endpunktadresse** die ngrok-URL ein, z. B. , wo Sie den Bot hosten und `https://287a4f4223bc.ngrok.io` an ihn `/api/messages` anfügen:
    ...
    Ihr Bot kann auf Nachrichten in Teams reagieren, nachdem Sie den Endpunkt ordnungsgemäß eingerichtet haben. 
...
* Senden Sie im Feld Verfassen eine `Hello` Nachricht.
    ...
    Sie haben nun einen einfachen Teams-Bot erstellt, der mit Benutzern 1:1 oder in Gruppeneinstellungen (Kanäle und Chats) kommunizieren 🎉
