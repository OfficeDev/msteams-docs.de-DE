...
| `url` | Zeichenfolge | Dieses Attribut ist die URL der Seite, die als `<iframe>` innerhalb des Aufgabenmoduls geladen wurde. Die Domäne der URL muss sich im [ValidDomains-Array](~/resources/schema/manifest-schema.md#validdomains) der App im Manifest Ihrer App befinden. |
...
| `fallbackUrl` | Zeichenfolge | Dieses Attribut öffnet die URL in einer Browserregisterkarte, wenn ein Client das Aufgabenmodulfeature nicht unterstützt. |
| `completionBotId` | Zeichenfolge | Dieses Attribut gibt eine Bot-App-ID an, um das Ergebnis der Benutzerinteraktion mit dem Aufgabenmodul zu senden. Wenn angegeben, empfängt der Bot ein `task/submit invoke` Ereignis mit einem JSON-Objekt in der Ereignisnutzlast. |
...
| `BOT_APP_ID` | Zeichenfolge | Nein | Wenn ein Wert für `completionBotId` angegeben wird, wird das `result` Objekt mithilfe einer Nachricht an den `task/submit invoke` angegebenen Bot gesendet. `BOT_APP_ID` muss im Manifest der App als Bot angegeben werden, d. h., Sie können ihn nicht an einen Bot senden. |
...
* [Integrieren der QR- oder Strichcodescannerfunktion in Teams](~/concepts/device-capabilities/qr-barcode-scanner-capability.md)
