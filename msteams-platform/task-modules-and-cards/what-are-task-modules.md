...
Aufgabenmodule aufbauen auf der Grundlage Microsoft Teams Registerkarten; Sie sind im Wesentlichen eine Registerkarte innerhalb eines Popupfensters. Sie verwenden dasselbe SDK. Wenn Sie also eine Registerkarte erstellt haben, sind Sie bereits zu 90 % in der Lage, ein Aufgabenmodul zu erstellen.
...
1. Der Name Ihrer [ `short` App](~/resources/schema/manifest-schema.md#name).
1. Der Titel des Aufgabenmoduls, der in der `title` Eigenschaft des [TaskInfo-Objekts angegeben ist.](#the-taskinfo-object)
1. Die Schaltfläche "Schließen/Abbrechen" des Aufgabenmoduls. Wenn der Benutzer dies drückt, erhält Ihre App ein `err` Ereignis wie hier [beschrieben.](~/task-modules-and-cards/task-modules/task-modules-tabs.md#example-submitting-the-result-of-a-task-module)
    > [!Note]
    > Es ist derzeit nicht möglich, dieses Ereignis zu erkennen, wenn ein Aufgabenmodul von einem Bot aufgerufen wird.
1. Das blaue Rechteck ist die Stelle, an der Ihre Webseite angezeigt wird, wenn Sie eine eigene Webseite mit der Eigenschaft des `url` [TaskInfo-Objekts laden.](#the-taskinfo-object) Weitere Details finden Sie im Abschnitt [zur Größenanpassung des](#task-module-sizing) Aufgabenmoduls weiter unten.
1. Wenn Sie eine adaptive Karte über die Eigenschaft des TaskInfo-Objekts anzeigen, wird der Abstand für Sie hinzugefügt, andernfalls müssen Sie dies `card` [selbst behandeln.](#task-module-css-for-htmljavascript-task-modules) [](#the-taskinfo-object)
1. Schaltflächen für adaptive Karten werden hier gerendert. Wenn Sie Eine eigene Seite verwenden, müssen Sie eigene Schaltflächen erstellen.
...
Microsoft Teams stellt sicher, dass die Tastaturnavigation vom Aufgabenmodulheader in Den HTML ordnungsgemäß funktioniert und umgekehrt.
...
* [Integrieren von QR- oder Barcodescannerfunktionen in Teams](../concepts/device-capabilities/qr-barcode-scanner-capability.md)
