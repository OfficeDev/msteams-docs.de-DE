...
1. <span data-ttu-id="f16b9-147">Wählen Sie oben links im angezeigten Fenster die Registerkarte **"Hinzufügen"** aus, um eine neue Ressourcengruppe zu erstellen.</span><span class="sxs-lookup"><span data-stu-id="f16b9-147">In the upper left of the displayed window, select **Add** tab to create a new resource group.</span></span> <span data-ttu-id="f16b9-148">Sie werden aufgefordert, Folgendes anzugeben:</span><span class="sxs-lookup"><span data-stu-id="f16b9-148">You'll be prompted to provide the following:</span></span>
    ...
    1. <span data-ttu-id="f16b9-156">Wählen Sie die Schaltfläche **"Überprüfen" und "Erstellen"** aus.</span><span class="sxs-lookup"><span data-stu-id="f16b9-156">Select the **Review and create** button.</span></span> <span data-ttu-id="f16b9-157">Es sollte ein Banner angezeigt werden, in dem die *Validierung gelesen wird.*</span><span class="sxs-lookup"><span data-stu-id="f16b9-157">You should see a banner that reads *Validation passed*.</span></span>
...
<span data-ttu-id="f16b9-195">Weitere Informationen finden Sie unter [Erstellen eines Bots für Teams](../create-a-bot-for-teams.md).</span><span class="sxs-lookup"><span data-stu-id="f16b9-195">For more information, see [Create a bot for Teams](../create-a-bot-for-teams.md).</span></span>
...
1. <span data-ttu-id="f16b9-204">Sie werden aufgefordert, die folgenden Informationen anzugeben:</span><span class="sxs-lookup"><span data-stu-id="f16b9-204">You'll be asked to provide the following information:</span></span>
   ...
   1. <span data-ttu-id="f16b9-214">Wählen Sie **Registrieren** aus.</span><span class="sxs-lookup"><span data-stu-id="f16b9-214">Select **Register**.</span></span>
...
1. <span data-ttu-id="f16b9-349">Wählen Sie im Dialogfeld auf der linken Seite **"App-Dienst"** und auf der rechten Seite **"Neu erstellen"** aus.</span><span class="sxs-lookup"><span data-stu-id="f16b9-349">In the dialog window, select **App Service** on the left and **Create New** on the right.</span></span>
...
## <a name="test-the-bot-using-the-emulator"></a><span data-ttu-id="f16b9-368">Testen des Bots mithilfe der Emulator</span><span class="sxs-lookup"><span data-stu-id="f16b9-368">Test the bot using the Emulator</span></span>
...
<span data-ttu-id="f16b9-383">Wenn der Bot eine Anmeldekarte anzeigt und der Benutzer die Anmeldeschaltfläche auswählt, öffnet das Emulator eine Seite, die der Benutzer zum Anmelden beim Authentifizierungsanbieter verwenden kann.</span><span class="sxs-lookup"><span data-stu-id="f16b9-383">When the bot displays a sign-in card and the user selects the sign-in button, the Emulator opens a page that the user can use to sign in with the authentication provider.</span></span>
...
1. <span data-ttu-id="f16b9-402">Je nachdem, welche Konfiguration Sie für die Emulator verwendet haben, erhalten Sie eine der folgenden Optionen:</span><span class="sxs-lookup"><span data-stu-id="f16b9-402">Depending which configuration you used for the Emulator, you get one of the following:</span></span>
    1. <span data-ttu-id="f16b9-403">**Verwenden des Anmeldeüberprüfungscodes**</span><span class="sxs-lookup"><span data-stu-id="f16b9-403">**Using sign-in verification code**</span></span>  
      <span data-ttu-id="f16b9-404">&#x2713; Wird ein Fenster geöffnet, in dem der Überprüfungscode angezeigt wird.</span><span class="sxs-lookup"><span data-stu-id="f16b9-404">&#x2713; A window is opened displaying the validation code.</span></span>  
...
1. <span data-ttu-id="f16b9-424">Ein Popupdialogfeld wird angezeigt, um die **Öffnen-URL** zu bestätigen.</span><span class="sxs-lookup"><span data-stu-id="f16b9-424">A pop-up dialog is displayed to **Confirm Open URL**.</span></span> <span data-ttu-id="f16b9-425">Dies ermöglicht die Authentifizierung des Bot-Benutzers (Sie).</span><span class="sxs-lookup"><span data-stu-id="f16b9-425">This is to allow the bot's user (you) to be authenticated.</span></span>  
...
1. <span data-ttu-id="f16b9-446">Wählen Sie die Dateien aus, und **zippen** Sie `manifest.json` `outline.png` `color.png` sie.</span><span class="sxs-lookup"><span data-stu-id="f16b9-446">Select and **zip** the `manifest.json`, `outline.png`, and `color.png` files.</span></span>
...
<span data-ttu-id="f16b9-461">Microsoft Teams ein vollständig cloudbasiertes Produkt ist, müssen alle Dienste, auf die es zugreift, über HTTPS-Endpunkte aus der Cloud verfügbar sein.</span><span class="sxs-lookup"><span data-stu-id="f16b9-461">Microsoft Teams is an entirely cloud-based product, it requires all services it accesses to be available from the cloud using HTTPS endpoints.</span></span> <span data-ttu-id="f16b9-462">Damit der Bot (unser Beispiel) in Teams funktioniert, müssen Sie den Code entweder in der Cloud Ihrer Wahl veröffentlichen oder über ein **Tunneling-Tool** extern auf eine lokal ausgeführte Instanz zugreifen.</span><span class="sxs-lookup"><span data-stu-id="f16b9-462">Therefore, to enable the bot (our sample) to work in Teams, you need to either publish the code to the cloud of your choice, or make a locally running instance externally accessible via a **tunneling** tool.</span></span> <span data-ttu-id="f16b9-463">Wir empfehlen  [ngrok](https://ngrok.com/download), das eine extern adressierbare URL für einen Port erstellt, den Sie lokal auf Ihrem Computer öffnen.</span><span class="sxs-lookup"><span data-stu-id="f16b9-463">We recommend  [ngrok](https://ngrok.com/download), which creates an externally addressable URL for a port you open locally on your machine.</span></span>
<span data-ttu-id="f16b9-464">Führen Sie die folgenden Schritte aus, um ngrok in Vorbereitung auf die lokale Ausführung Ihrer Microsoft Teams-App einzurichten:</span><span class="sxs-lookup"><span data-stu-id="f16b9-464">To set up ngrok in preparation for running your Microsoft Teams app locally, follow these steps:</span></span>

1. <span data-ttu-id="f16b9-465">Wechseln Sie in einem Terminalfenster zum Verzeichnis, in dem Sie `ngrok.exe` installiert haben.</span><span class="sxs-lookup"><span data-stu-id="f16b9-465">In a terminal window, go the directory where you have `ngrok.exe` installed.</span></span> <span data-ttu-id="f16b9-466">Es wird empfohlen, den Pfad der *Umgebungsvariablen* so festzulegen, dass er darauf zeigt.</span><span class="sxs-lookup"><span data-stu-id="f16b9-466">We suggest setting the *environment variable* path to point to it.</span></span>
1. <span data-ttu-id="f16b9-467">Führen Sie z. `ngrok http 3978 --host-header=localhost:3978` B. .</span><span class="sxs-lookup"><span data-stu-id="f16b9-467">Run, for example, `ngrok http 3978 --host-header=localhost:3978`.</span></span> <span data-ttu-id="f16b9-468">Ersetzen Sie die Portnummer nach Bedarf.</span><span class="sxs-lookup"><span data-stu-id="f16b9-468">Replace the port number as needed.</span></span>
<span data-ttu-id="f16b9-469">Dadurch wird ngrok gestartet, um den von Ihnen angegebenen Port abzuhören.</span><span class="sxs-lookup"><span data-stu-id="f16b9-469">This launches ngrok to listen on the port you specify.</span></span> <span data-ttu-id="f16b9-470">Im Gegenzug erhalten Sie eine extern adressierbare URL, die gültig ist, solange ngrok ausgeführt wird.</span><span class="sxs-lookup"><span data-stu-id="f16b9-470">In return, it gives you an externally addressable URL, valid for as long as ngrok is running.</span></span> <span data-ttu-id="f16b9-471">Die folgende Abbildung ist ein Beispiel:</span><span class="sxs-lookup"><span data-stu-id="f16b9-471">The following image is an example:</span></span>

    ![Teams-Bot-App- Authentifizierungs-Verbindungszeichenfolge adv1](../../../assets/images/authentication/auth-bot-ngrok-start.PNG)<span data-ttu-id="f16b9-473">.</span><span class="sxs-lookup"><span data-stu-id="f16b9-473">.</span></span>

1. <span data-ttu-id="f16b9-474">Kopieren Sie die HTTPS-Weiterleitungsadresse.</span><span class="sxs-lookup"><span data-stu-id="f16b9-474">Copy the forwarding HTTPS address.</span></span> <span data-ttu-id="f16b9-475">Es sollte etwa wie folgt aussehen: `https://dea822bf.ngrok.io/` .</span><span class="sxs-lookup"><span data-stu-id="f16b9-475">It should be similar to the following: `https://dea822bf.ngrok.io/`.</span></span>
1. <span data-ttu-id="f16b9-476">Append `/api/messages` to obtain `https://dea822bf.ngrok.io/api/messages` .</span><span class="sxs-lookup"><span data-stu-id="f16b9-476">Append `/api/messages` to obtain `https://dea822bf.ngrok.io/api/messages`.</span></span> <span data-ttu-id="f16b9-477">Dies ist der **Nachrichtenendpunkt** für den Bot, der lokal auf Ihrem Computer ausgeführt wird und in einem Chat in Microsoft Teams über das Web erreichbar ist.</span><span class="sxs-lookup"><span data-stu-id="f16b9-477">This is the **messages endpoint** for the bot running locally on your machine and reachable over the web in a chat in Microsoft Teams.</span></span>
1. <span data-ttu-id="f16b9-478">Ein letzter Schritt besteht darin, den Nachrichtenendpunkt des bereitgestellten Bots zu aktualisieren.</span><span class="sxs-lookup"><span data-stu-id="f16b9-478">One final step to perform is to update the messages endpoint of the deployed bot.</span></span> <span data-ttu-id="f16b9-479">In dem Beispiel haben wir den Bot in Azure bereitgestellt.</span><span class="sxs-lookup"><span data-stu-id="f16b9-479">In the example, we deployed the bot in Azure.</span></span> <span data-ttu-id="f16b9-480">\*\*Führen wir also die folgenden Schritte aus:</span><span class="sxs-lookup"><span data-stu-id="f16b9-480">So \*\*let's perform these steps:</span></span>
    1. <span data-ttu-id="f16b9-481">Navigieren Sie in Ihrem Browser zum [**Azure-Portal.**][azure-portal]</span><span class="sxs-lookup"><span data-stu-id="f16b9-481">In your browser navigate to the [**Azure portal**][azure-portal].</span></span>
    1. <span data-ttu-id="f16b9-482">Wählen Sie Ihre **Bot-Kanalregistrierung aus.**</span><span class="sxs-lookup"><span data-stu-id="f16b9-482">Select your **Bot Channel Registration**.</span></span>
    1. <span data-ttu-id="f16b9-483">Wählen Sie im linken Bereich **Einstellungen** aus.</span><span class="sxs-lookup"><span data-stu-id="f16b9-483">In the left panel, select **Settings**.</span></span>
    1. <span data-ttu-id="f16b9-484">Geben Sie im rechten Bereich im Feld **"Messaging-Endpunkt"** die ngrok-URL in unser Beispiel `https://dea822bf.ngrok.io/api/messages` ein.</span><span class="sxs-lookup"><span data-stu-id="f16b9-484">In the right panel, in the **Messaging endpoint** box, enter the ngrok URL, in our example, `https://dea822bf.ngrok.io/api/messages`.</span></span>
1. <span data-ttu-id="f16b9-485">Starten Sie Ihren Bot lokal, z. B. im Visual Studio Debugmodus.</span><span class="sxs-lookup"><span data-stu-id="f16b9-485">Start your bot locally, for example in Visual Studio debug mode.</span></span>
1. <span data-ttu-id="f16b9-486">Testen Sie den Bot während der lokalen Ausführung mithilfe des **Testwebchats** des Bot Framework-Portals.</span><span class="sxs-lookup"><span data-stu-id="f16b9-486">Test the bot while running locally using the Bot Framework portal's **Test Web chat**.</span></span> <span data-ttu-id="f16b9-487">Wie die Emulator können Sie bei diesem Test nicht auf Teams-spezifische Funktionalität zugreifen.</span><span class="sxs-lookup"><span data-stu-id="f16b9-487">Like the Emulator, this test doesn't allow you to access Teams-specific functionality.</span></span>
1. <span data-ttu-id="f16b9-488">Im Terminalfenster, in dem `ngrok` ausgeführt wird, können Sie HTTP-Datenverkehr zwischen dem Bot und dem Webchat-Client anzeigen.</span><span class="sxs-lookup"><span data-stu-id="f16b9-488">In the terminal window where `ngrok` is running you can see HTTP traffic between the bot and the web chat client.</span></span> <span data-ttu-id="f16b9-489">Wenn Sie eine detailliertere Ansicht wünschen, geben Sie in einem Browserfenster `http://127.0.0.1:4040` die aus dem vorherigen Terminalfenster abgerufene Ansicht ein.</span><span class="sxs-lookup"><span data-stu-id="f16b9-489">If you want a more detailed view, in a browser window enter `http://127.0.0.1:4040` you obtained from the previous terminal window.</span></span> <span data-ttu-id="f16b9-490">Die folgende Abbildung ist ein Beispiel:</span><span class="sxs-lookup"><span data-stu-id="f16b9-490">The following image is an example:</span></span>

    ![auth bot teams ngrok testing](../../../assets/images/authentication/auth-bot-teams-ngrok-testing.png)<span data-ttu-id="f16b9-492">.</span><span class="sxs-lookup"><span data-stu-id="f16b9-492">.</span></span>

> [!NOTE]
> <span data-ttu-id="f16b9-493">Wenn Sie ngrok beenden und neu starten, ändert sich die URL.</span><span class="sxs-lookup"><span data-stu-id="f16b9-493">If you stop and restart ngrok, the URL changes.</span></span> <span data-ttu-id="f16b9-494">Um ngrok in Ihrem Projekt zu verwenden, und abhängig von den von Ihnen verwendeten Funktionen, müssen Sie alle URL-Verweise aktualisieren.</span><span class="sxs-lookup"><span data-stu-id="f16b9-494">To use ngrok in your project, and depending on the capabilities you're using, you must update all URL references.</span></span>
 

## <a name="additional-information"></a><span data-ttu-id="f16b9-495">Weitere Informationen</span><span class="sxs-lookup"><span data-stu-id="f16b9-495">Additional information</span></span>

### <a name="teamsappmanifestmanifestjson"></a><span data-ttu-id="f16b9-496">TeamsAppManifest/manifest.json</span><span class="sxs-lookup"><span data-stu-id="f16b9-496">TeamsAppManifest/manifest.json</span></span>

<span data-ttu-id="f16b9-497">Dieses Manifest enthält Informationen, die von Microsoft Teams benötigt werden, um eine Verbindung mit dem Bot herzustellen:</span><span class="sxs-lookup"><span data-stu-id="f16b9-497">This manifest contains information needed by Microsoft Teams to connect with the bot:</span></span>  
...
<span data-ttu-id="f16b9-498">Bei der Authentifizierung verhält sich Teams etwas anders als bei anderen Kanälen, wie unten erläutert.</span><span class="sxs-lookup"><span data-stu-id="f16b9-498">With authentication, Teams behaves slightly differently than other channels, as explained below.</span></span>

### <a name="handling-invoke-activity"></a><span data-ttu-id="f16b9-499">Behandeln von Aufrufaktivitäten</span><span class="sxs-lookup"><span data-stu-id="f16b9-499">Handling Invoke Activity</span></span>

<span data-ttu-id="f16b9-500">Eine **Aufrufaktivität** wird an den Bot gesendet und nicht an die Ereignisaktivität, die von anderen Kanälen verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="f16b9-500">An **Invoke Activity** is sent to the bot rather than the Event Activity used by other channels.</span></span>
<span data-ttu-id="f16b9-501">Dies geschieht durch Unterklassen des **ActivityHandler**.</span><span class="sxs-lookup"><span data-stu-id="f16b9-501">This is done by sub-classing the **ActivityHandler**.</span></span>

# <a name="cnet"></a>[<span data-ttu-id="f16b9-502">C#/.NET</span><span class="sxs-lookup"><span data-stu-id="f16b9-502">C#/.NET</span></span>](#tab/dotnet-sample)

<span data-ttu-id="f16b9-503">**Bots/DialogBot.cs**</span><span class="sxs-lookup"><span data-stu-id="f16b9-503">**Bots/DialogBot.cs**</span></span>

[!code-csharp[ActivityHandler](~/../botbuilder-samples/samples/csharp_dotnetcore/46.teams-auth/Bots/DialogBot.cs?range=19-51)]

<span data-ttu-id="f16b9-504">**Bots/TeamsBot.cs**</span><span class="sxs-lookup"><span data-stu-id="f16b9-504">**Bots/TeamsBot.cs**</span></span>

<span data-ttu-id="f16b9-505">Die *Aufrufaktivität* muss an das Dialogfeld weitergeleitet werden, wenn **OAuthPrompt** verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="f16b9-505">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-csharp[ActivityHandler](~/../botbuilder-samples/samples/csharp_dotnetcore/46.teams-auth/Bots/TeamsBot.cs?range=34-42)]

#### <a name="teamsactivityhandlercs"></a><span data-ttu-id="f16b9-506">TeamsActivityHandler.cs</span><span class="sxs-lookup"><span data-stu-id="f16b9-506">TeamsActivityHandler.cs</span></span>
...
# <a name="javascript"></a>[<span data-ttu-id="f16b9-507">JavaScript</span><span class="sxs-lookup"><span data-stu-id="f16b9-507">JavaScript</span></span>](#tab/node-js-dialog-sample)

<span data-ttu-id="f16b9-508">**Bots/dialogBot.js**</span><span class="sxs-lookup"><span data-stu-id="f16b9-508">**bots/dialogBot.js**</span></span>

[!code-javascript[ActivityHandler](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/bots/dialogBot.js?range=4-46)]

<span data-ttu-id="f16b9-509">**Bots/teamsBot.js**</span><span class="sxs-lookup"><span data-stu-id="f16b9-509">**bots/teamsBot.js**</span></span>

<span data-ttu-id="f16b9-510">Die *Aufrufaktivität* muss an das Dialogfeld weitergeleitet werden, wenn **OAuthPrompt** verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="f16b9-510">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-javascript[ActivityHandler](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/bots/teamsBot.js?range=4-33)]

<span data-ttu-id="f16b9-511">**Dialogfelder/mainDialog.js**</span><span class="sxs-lookup"><span data-stu-id="f16b9-511">**dialogs/mainDialog.js**</span></span>

<span data-ttu-id="f16b9-512">Starten Sie in einem Dialogfeld `beginDialog` die OAuth-Eingabeaufforderung, in der der Benutzer zur Anmeldung aufgefordert wird.</span><span class="sxs-lookup"><span data-stu-id="f16b9-512">Within a dialog step, use `beginDialog` to start the OAuth prompt, which asks the user to sign in.</span></span>

- <span data-ttu-id="f16b9-513">Wenn der Benutzer bereits angemeldet ist, generiert dies ein Tokenantwortereignis, ohne den Benutzer aufzufordern.</span><span class="sxs-lookup"><span data-stu-id="f16b9-513">If the user is already signed in, this will generate a token response event, without prompting the user.</span></span>
- <span data-ttu-id="f16b9-514">Andernfalls wird der Benutzer aufgefordert, sich anzumelden.</span><span class="sxs-lookup"><span data-stu-id="f16b9-514">Otherwise, this will prompt the user to sign in.</span></span> <span data-ttu-id="f16b9-515">Der Azure Bot Service sendet das Tokenantwortereignis, nachdem der Benutzer versucht, sich anzumelden.</span><span class="sxs-lookup"><span data-stu-id="f16b9-515">The Azure Bot Service sends the token response event after the user attempts to sign in.</span></span>

[!code-javascript[AddOAuthPrompt](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/mainDialog.js?range=50-52)]

<span data-ttu-id="f16b9-516">Überprüfen Sie im folgenden Dialogfeldschritt, ob im Ergebnis des vorherigen Schritts ein Token vorhanden ist.</span><span class="sxs-lookup"><span data-stu-id="f16b9-516">Within the following dialog step, check for the presence of a token in the result from the previous step.</span></span> <span data-ttu-id="f16b9-517">Wenn der Wert nicht NULL ist, hat sich der Benutzer erfolgreich angemeldet.</span><span class="sxs-lookup"><span data-stu-id="f16b9-517">If it is not null, the user successfully signed in.</span></span>

[!code-javascript[AddOAuthPrompt](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/mainDialog.js?range=50-64)]

<span data-ttu-id="f16b9-518">**Bots/logoutDialog.js**</span><span class="sxs-lookup"><span data-stu-id="f16b9-518">**bots/logoutDialog.js**</span></span>

[!code-javascript[allow-logout](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/logoutDialog.js?range=31-42&highlight=7)]

# <a name="python"></a>[<span data-ttu-id="f16b9-519">Python</span><span class="sxs-lookup"><span data-stu-id="f16b9-519">Python</span></span>](#tab/python-sample)

<span data-ttu-id="f16b9-520">**bots/dialog_bot.py**</span><span class="sxs-lookup"><span data-stu-id="f16b9-520">**bots/dialog_bot.py**</span></span>

[!code-python[ActivityHandler](~/../botbuilder-samples/samples/python/46.teams-auth/bots/dialog_bot.py?range=10-42)]

<span data-ttu-id="f16b9-521">**bots/teams_bot.py**</span><span class="sxs-lookup"><span data-stu-id="f16b9-521">**bots/teams_bot.py**</span></span>

<span data-ttu-id="f16b9-522">Die *Aufrufaktivität* muss an das Dialogfeld weitergeleitet werden, wenn **OAuthPrompt** verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="f16b9-522">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-python[on_token_response_event](~/../botbuilder-samples/samples/python/46.teams-auth/bots/teams_bot.py?range=38-45)]

<span data-ttu-id="f16b9-523">**dialogs/main_dialog.py**</span><span class="sxs-lookup"><span data-stu-id="f16b9-523">**dialogs/main_dialog.py**</span></span>

<span data-ttu-id="f16b9-524">Starten Sie in einem Dialogfeld `begin_dialog` die OAuth-Eingabeaufforderung, in der der Benutzer zur Anmeldung aufgefordert wird.</span><span class="sxs-lookup"><span data-stu-id="f16b9-524">Within a dialog step, use `begin_dialog` to start the OAuth prompt, which asks the user to sign in.</span></span>

- <span data-ttu-id="f16b9-525">Wenn der Benutzer bereits angemeldet ist, generiert dies ein Tokenantwortereignis, ohne den Benutzer aufzufordern.</span><span class="sxs-lookup"><span data-stu-id="f16b9-525">If the user is already signed in, this will generate a token response event, without prompting the user.</span></span>
- <span data-ttu-id="f16b9-526">Andernfalls wird der Benutzer aufgefordert, sich anzumelden.</span><span class="sxs-lookup"><span data-stu-id="f16b9-526">Otherwise, this will prompt the user to sign in.</span></span> <span data-ttu-id="f16b9-527">Der Azure Bot Service sendet das Tokenantwortereignis, nachdem der Benutzer versucht, sich anzumelden.</span><span class="sxs-lookup"><span data-stu-id="f16b9-527">The Azure Bot Service sends the token response event after the user attempts to sign in.</span></span>

[!code-python[Add OAuthPrompt](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/main_dialog.py?range=48-49)]

<span data-ttu-id="f16b9-528">Überprüfen Sie im folgenden Dialogfeldschritt, ob im Ergebnis des vorherigen Schritts ein Token vorhanden ist.</span><span class="sxs-lookup"><span data-stu-id="f16b9-528">Within the following dialog step, check for the presence of a token in the result from the previous step.</span></span> <span data-ttu-id="f16b9-529">Wenn der Wert nicht NULL ist, hat sich der Benutzer erfolgreich angemeldet.</span><span class="sxs-lookup"><span data-stu-id="f16b9-529">If it is not null, the user successfully signed in.</span></span>

[!code-python[Add OAuthPrompt](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/main_dialog.py?range=51-61)]

<span data-ttu-id="f16b9-530">**dialogs/logout_dialog.py**</span><span class="sxs-lookup"><span data-stu-id="f16b9-530">**dialogs/logout_dialog.py**</span></span>

[!code-python[allow logout](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/logout_dialog.py?range=29-36&highlight=6)]

---

## <a name="see-also"></a><span data-ttu-id="f16b9-531">Siehe auch</span><span class="sxs-lookup"><span data-stu-id="f16b9-531">See also</span></span>

[<span data-ttu-id="f16b9-532">Hinzufügen der Authentifizierung über Azure Bot Service</span><span class="sxs-lookup"><span data-stu-id="f16b9-532">Add authentication through Azure Bot Service</span></span>](https://aka.ms/azure-bot-add-authentication)
