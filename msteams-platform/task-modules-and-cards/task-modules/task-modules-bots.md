...
Wenn der Benutzer mit dem Aufgabenmodul fertig ist, ähnelt das Senden des Ergebnisses an den Bot der Art und Weise, wie es mit Registerkarten funktioniert. Weitere Informationen finden Sie im [Beispiel zum Übermitteln des Ergebnisses eines Aufgabenmoduls](~/task-modules-and-cards/task-modules/task-modules-tabs.md#example-of-submitting-the-result-of-a-task-module). Es gibt einige Unterschiede wie folgt:
...
> [!NOTE]
> Das Verketten adaptiver Karten in eine Sequenz ist ein erweitertes Szenario. Die Node.js-Beispiel-App unterstützt dies. Weitere Informationen finden Sie [unter Microsoft Teams Aufgabenmodul Node.js](https://github.com/OfficeDev/microsoft-teams-sample-task-module-nodejs#implementation-notes).
...
| `value`  | Ist die vom Entwickler definierte Nutzlast. Die Struktur des `value` Objekts entspricht der Struktur, die von Teams gesendet wird. In diesem Fall ist es jedoch anders. Es erfordert Unterstützung für dynamisches Abrufen, das `task/fetch` sowohl vom Bot-Framework als `value` auch von Aktionen für adaptive Karten `Action.Submit` stammt, d `data`. h. . Eine Möglichkeit, Teams `context` mit dem Bot zu kommunizieren, ist zusätzlich zu dem, was in `value` oder `data`enthalten ist, erforderlich.<br/><br/>Kombinieren Sie "Wert" und "Daten" in einem übergeordneten Objekt:<br/><br/><pre>{<br/>  "context": {<br/>    "theme": "default" &vert; "dark" &vert; "contrast",<br/>  },<br/>  "data": [value field from Bot Framework card] &vert; [data field from Adaptive Card] <br/>}</pre>  |

Der nächste Abschnitt enthält ein Beispiel für das Empfangen und Beantworten und Aufrufen von Nachrichten in Node.js.The next section provides an example of receiving and responding to `task/fetch` and `task/submit` invoke messages in Node.js.
