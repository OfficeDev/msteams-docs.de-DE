...
1. Wählen Sie die **App Studio-Kachel** aus, und wählen Sie **"Installieren**" aus. App Studio ist installiert:
...
1. Um das App-Paket für Ihre Teams App zu erstellen, wählen Sie die Registerkarte "**Manifest-Editor**" in **App Studio** aus:
    ...
    Das Beispiel enthält ein eigenes Manifest und ist so konzipiert, dass beim Erstellen des Projekts ein App-Paket erstellt wird. Bei Node.js erfolgt dies `gulp` durch Eingabe an der Befehlszeile im Stammverzeichnis des Projekts.

    Sie können das App-Paket auf Node.js erstellen, indem Sie die Eingabe über die Befehlszeile im Stammverzeichnis des Projekts ausführen `gulp` .
...
1. Geben Sie im Dialogfeld **"Neu"** die folgenden Werte ein:
    ...
    * **Beschreibung**: Eingeben einer zufälligen Beschreibung
...
* Die Autorisierungsanmeldeinformationen für Ihren Bot sind wie folgt:
  ...
  * MICROSOFT_APP_PASSWORD Kennwort ist
* NODE_DEBUG Ihnen zeigen, was in Ihrem Bot in der Visual Studio Code Debugkonsole passiert
