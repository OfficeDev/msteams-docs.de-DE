...
| 4 | Azure AD → Teams-Client | Azure AD sendet das Zugriffstoken an den Teams-Client. Das Token ist ein JSON-Webtoken (JWT), und seine Überprüfung funktioniert genau wie die Tokenüberprüfung in den meisten standardmäßigen OAuth-Flüssen. Teams speichert das Token in Ihrem Auftrag zwischen, sodass zukünftige Aufrufe das `getAuthToken()` zwischengespeicherte Token zurückgeben. |
| 5 | Teams-Client → Registerkarten-App-Client | Teams sendet das Zugriffstoken als Teil des ergebnisobjekts, das vom Aufruf zurückgegeben wird, an die Registerkarten-App `getAuthToken()` . |
| 6  | Tab-App (zwischen Client & Server) | Die Registerkarten-App analysiert das Zugriffstoken mit JavaScript, um erforderliche Informationen zu extrahieren, z. B. die E-Mail-Adresse des App-Benutzers. Das an die Registerkarten-App zurückgegebene Token ist sowohl ein Zugriffstoken als auch ein Identitätstoken. |

Weitere Informationen finden Sie unter ["Updatecode zum Aktivieren von SSO"](tab-sso-code.md).

> [!IMPORTANT]
> Dies `getAuthToken()` gilt nur für die Zustimmung zu einer begrenzten Gruppe von APIs auf Benutzerebene, z. B. E-Mail, Profil, offline_access und OpenId. Es wird nicht für andere Graph-Bereiche wie `User.Read` oder `Mail.Read`verwendet. Vorgeschlagene Problemumgehungen finden Sie unter [Erweitern Ihrer App mit Microsoft Graph-Berechtigungen](tab-sso-graph-api.md).
...
## <a name="enable-sso-for-a-tab-app"></a>Aktivieren von SSO für eine Registerkarten-App
...
### <a name="teams-mobile-client-support"></a>Unterstützung mobiler Teams-Clients
...
- Teams JavaScript SDK (Version: 1.11 und höher) für SSO zur Arbeit im Besprechungsbereich
...
- Mehrere Domänen pro App werden nicht unterstützt. Weitere Informationen finden Sie unter [Branchen-Apps](tab-sso-register-aad.md#before-you-register-with-azure-ad).

## <a name="next-step"></a>Nächster Schritt

> [!div class="nextstepaction"]
> [Registrieren Ihrer Registerkartenanwendung in Azure AD](tab-sso-register-aad.md)
