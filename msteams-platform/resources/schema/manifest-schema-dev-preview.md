...
Der eindeutige von Microsoft generierte Bezeichner für diese App. Wenn Sie einen Bot über die Microsoft Bot Framework registriert haben oder sich die Web-App Ihrer Registerkarte bereits bei Microsoft anmeldet, sollten Sie bereits über eine ID verfügen und sie hier eingeben. Andernfalls sollten Sie eine neue ID im Microsoft-Anwendungsregistrierungsportal ([Meine Anwendungen](https://apps.dev.microsoft.com)) generieren, sie hier eingeben und sie dann wiederverwenden, wenn Sie [einen Bot hinzufügen](~/bots/how-to/create-a-bot-for-teams.md).
...
Rendern Sie Registerkarten mit adaptiven Karten, indem Sie anstelle des `contentUrl` **staticTabs-Blocks** angeben`contentBotId`.
...
Das Objekt ist ein Array (maximal 1 Element&mdash; ist pro App nur ein Bot zulässig) mit allen Elementen des Typs `object`. Dieser Block ist nur für Lösungen erforderlich, die einen Bot umfassen.
...
Ihre Messaging-Erweiterung sollte einen oder mehrere Befehle deklarieren. Jeder Befehl wird in Microsoft Teams als potenzielle Interaktion vom Einstiegspunkt in der Benutzeroberfläche angezeigt. Es gibt maximal 10 Befehle.
