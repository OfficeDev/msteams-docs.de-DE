...
Damit IT-Administratoren diese Integration problemlos einrichten können, wird open-source Microsoft 365 Das Moodle-Plug-In mit den folgenden Funktionen aktualisiert:
...
Die Integration von Moodle in Microsoft Teams wird von der Open Source-Microsoft 365 [von "Moodle"-Plug-Ins unterstützt.](https://github.com/Microsoft/o365-moodle) Zum Installieren des Plug-Ins auf Ihrem Moodle-Server müssen die folgenden Anwendungen installiert sein:

1. Eine [aktuelle stabile Version von Moodle](https://download.moodle.org/releases/latest/).

1. Die Moodle [OpenID Verbinden](https://moodle.org/plugins/auth_oidc) und die Microsoft 365 Integrations-Plug-Ins heruntergeladen und auf Ihrem lokalen Computer gespeichert. [](https://moodle.org/plugins/local_o365)

   > [!NOTE]
   > Die Installation der OpenID-Verbinden und Microsoft 365 Integrations-Plug-Ins sind für die Integration Teams erforderlich. Darüber hinaus wird das Microsoft 365 Teams Design-Plug-In dringend empfohlen. [](https://moodle.org/plugins/theme_boost_o365teams)
...
1. Wählen Hochladen der linken **Navigationsleiste** eine Dateioption aus, suchen Sie nach der heruntergeladenen Datei, und wählen Sie **Hochladen datei aus.**

1. Wählen Sie **die Websiteverwaltung** im linken Navigationsbereich aus, um zum Administratordashboard zurückzukehren. Scrollen Sie nach unten zu **den lokalen Plug-Ins,** und wählen **Sie den Link Microsoft 365 Integration** aus. 

> [!IMPORTANT]
>
> * Lassen Sie Microsoft 365 Die Konfigurationsseite des Moodle-Plug-Ins auf einer separaten Browserregisterkarte geöffnet, da Sie während des gesamten Prozesses zu diesem Seitensatz zurückkehren müssen.  <br/><br/>
...
Sie müssen Moodle als Anwendung in Azure AD registrieren. Verwenden Sie das PowerShell-Skript, um diesen Prozess auszuführen. Das PowerShell-Skript bietet eine neue Azure AD-Anwendung für Ihren Microsoft 365 Mandanten, die vom Microsoft 365 Verwendet wird. Das Skript bietet die App für Microsoft 365 Mandanten, richten Sie die erforderlichen Antwort-URLs und Berechtigungen für die bereitgestellte App ein, und gibt die `AppID` und `Key` zurück. Sie können die generierte und in Ihrer Microsoft 365 Einstellungsseite des Moodle-Plug-Ins verwenden, um Ihre `AppID` `Key` Moodle-Serverwebsite mit Azure AD zu konfigurieren.
...
1. Wählen Sie auf Microsoft 365 Seite Integrations-Plug-In die **Registerkarte Setup** aus.
...
1. Als Nächstes müssen Sie das `AppID` und dem Microsoft 365 `Key` Hinzufügen. Kehren Sie zur Seite für die Verwaltung des Plug-Ins zurück. Der Ablauf ist: Websiteverwaltung ➡ Plugins ➡ Microsoft 365 Integration.
...
1. Synchronisieren Sie Benutzer zwischen Ihrem Moodle-Server und Azure AD. Je nach Umgebung können Sie während dieser Phase verschiedene Optionen auswählen. Erste Schritte:
    1. Wechseln zur Registerkarte **Einstellungen Synchronisieren**
...
1. Kehren Sie zur Seite für die Verwaltung des Plug-Ins zurück. Der Navigationsfluss ist: Websiteverwaltung ➡ Plugins ➡ Microsoft 365 Integratio. Wählen Sie dann die Seite **Teams Einstellungen** aus. Sie müssen einige Einstellungen konfigurieren, um die Integration der Teams zu aktivieren.
...
1. Wechseln Sie zur Seite für die Verwaltung des Plug-Ins. Wechseln Sie zu **Plugins**. Wählen **Microsoft 365 Integration** die Registerkarte **Teams Einstellungen** aus.
...
> [!div class="checklist"]
> * Wählen Sie **die Schaltfläche Bereitstellen in Azure** aus, und füllen Sie das Formular mit den erforderlichen Informationen aus, z. B. die Botanwendungs-ID, das Botanwendungskennwort und das Geheime Kennwort von Moodle befinden sich auf der Seite Team Einstellungen Seite.  Die Azure-Informationen finden Sie auf der **Seite Setup).** 
...
Nachdem die Bereitstellung der Ressourcen in Azure abgeschlossen ist, müssen Sie das Microsoft 365 Von Moodle mit einem Messagingendpunkt konfigurieren. Sie müssen den Endpunkt von Ihrem Bot in Azure erhalten:

**Konfigurieren des Microsoft 365 von "Moodle" mit einem Messagingendpunkt**  
...
1. Öffnen Sie in Moodle die **Seite Team Einstellungen** Ihres Microsoft 365 -Plug-Ins.
...
4. Wählen Sie das `manifest.zip` paket aus, das Sie zuvor heruntergeladen haben, und wählen Sie **Speichern aus.** Wenn Sie das App-Manifestpaket nicht heruntergeladen haben, können Sie auf der Registerkarte **Team Einstellungen** der Plug-In-Konfigurationsseite in Moodle herunterladen.
...
Obwohl die Registerkarten "Moodle" manuell in Microsoft Teams erstellt werden, können Sie sie automatisch erstellen lassen, wenn Teams aus der Kurssynchronisierung erstellt werden. Dazu müssen Sie die ID der hochgeladenen app Microsoft Teams in Moodle konfigurieren:
...
1. Öffnen Sie in Moodle die **Registerkarte Teams Der App** auf ihrer Microsoft 365 -Konfigurationsseite des Moodle-Plug-Ins.
