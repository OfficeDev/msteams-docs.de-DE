...
Ihr Bot kann Nachrichten nach dem Senden dynamisch aktualisieren, anstatt sie als statische Momentaufnahmen von Daten zu verwenden. Nachrichten können auch mithilfe der Bot Framework-Methode `DeleteActivity` gelöscht werden.
...
Um eine vorhandene Nachricht zu aktualisieren, übergeben Sie ein neues `Activity` Objekt mit der vorhandenen Aktivitäts-ID an die `UpdateActivityAsync` Methode der `TurnContext` Klasse. Weitere Informationen finden Sie unter [TurnContextClass](/dotnet/api/microsoft.bot.builder.turncontext?view=botbuilder-dotnet-stable&preserve-view=true).
...
Um die vorhandene Karte auf der Schaltflächenauswahl zu aktualisieren, können Sie eingehende Aktivitäten verwenden `ReplyToId` .
...
Im Bot Framework verfügt jede Nachricht über einen eindeutigen Aktivitätsbezeichner. Nachrichten können mithilfe der Bot Framework-Methode `DeleteActivity` gelöscht werden.
